...
const Home = () => {
  ...
  const getBillList = async () => {
    const { data } = await get(`/bill/get_bill_list?date=${currentTime}&type_id=${currentSelect.id || 'all'}&page=${page}&page_size=5`);
    // 下拉刷新，重制数据
    if (page == 1) {
      setList(data.list);
    } else {
    ...
    setTotalExpense(data.totalMonthExpense.toFixed(2));
    setTotalIncome(data.totalMonthIncome.toFixed(2));
